...
export const OversiktScreen: React.FC = () => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.base }]}>
      <AppBar title="Oversikt" scrollY={scrollY} />
      ...
      >
          ...
          <Card style={styles.phaseCard}>
              ...
              <View style={styles.phaseDotsRow}>
                {phases.map((p, i) => (
                  <View key={i} style={styles.phaseDotContainer}>
                    <View style={[styles.phaseDot, { backgroundColor: p.done || p.active ? theme.accent1 : theme.surface, borderColor: p.done || p.active ? theme.accent1 : theme.hairline, width: p.active ? 18 : 11, height: p.active ? 18 : 11, borderRadius: 9 }]} />
                  </View>
                ))}
              </View>
            </View>
            <View style={styles.phaseLabelsRow}>
              {phases.map((p, i) => (
                <View key={i} style={styles.phaseLabelContainer}>
                  <Text style={[styles.phaseLabelText, { color: p.active ? theme.text : theme.textFaint, fontWeight: p.active ? '600' : '400' }]}>{p.label}</Text>
            ...
            <View style={[styles.phaseInfo, { borderTopColor: theme.hairline }]}>
              <Text style={[styles.phaseInfoText, { color: theme.textMuted }]}>
                <Text style={[styles.phaseInfoHighlight, { color: theme.text }]}>{stats.days < 7 ? 'Akutt fase. ' : stats.days < 14 ? 'Tidlig fase. ' : 'Oppbygging. '}</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.patternsGrid}>
            {[{ title: 'Triggere som drar', segs: [{ v: 40, c: theme.crisis, l: 'Kveld alene' }, { v: 28, c: theme.accent1, l: 'Fest' }, { v: 18, c: theme.textMuted, l: 'Stress' }, { v: 14, c: theme.hairline, l: 'Annet' }]}, { title: 'Som hjelper', segs: [{ v: 34, c: theme.accent2, l: 'Tur' }, { v: 30, c: theme.primary, l: 'Ring Siri' }, { v: 22, c: theme.accent1, l: 'Dusj + søvn' }, { v: 14, c: theme.hairline, l: 'Annet' }]}].map((g, i) => (
              <Card key={i} style={styles.patternCard}>
                <Text style={[styles.patternTitle, { color: theme.textMuted }]}>{g.title}</Text>
                <View style={styles.pieContainer}><PieChart segments={g.segs} size={100} /></View>
                <View style={styles.legend}>{g.segs.slice(0, 3).map((s, j) => (<View key={j} style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: s.c }]} /><Text style={[styles.legendText, { color: theme.textMuted }]} numberOfLines={1}>{s.l}</Text></View>))}</View>
              </Card>
            ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  phaseCard: { padding: 22, paddingHorizontal: 18 },
  phaseTimeline: { height: 40, justifyContent: 'center', position: 'relative' },
  timelineBase: { height: 3, borderRadius: 2, marginHorizontal: 8 },
  timelineProgress: { position: 'absolute', height: 3, borderRadius: 2, left: 8 },
  phaseDotsRow: { position: 'absolute', left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between' },
  phaseDotContainer: { width: 60, alignItems: 'center', justifyContent: 'center' },
  phaseDot: { borderWidth: 1.5 },
  phaseLabelsRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  phaseLabelContainer: { width: 60, alignItems: 'center' },
  phaseLabelText: { fontSize: 10 },
  phaseIdText: { fontSize: 9, marginTop: 2 },
  phaseInfo: { marginTop: 16, paddingTop: 16, borderTopWidth: 1 },
  phaseInfoText: { fontSize: 13, lineHeight: 20 },
  phaseInfoHighlight: { fontWeight: '600' },
  patternsGrid: { flexDirection: 'row', gap: 10 },
  patternCard: { flex: 1, padding: 14 },
  patternTitle: { fontSize: 12, marginBottom: 8 },
  pieContainer: { alignItems: 'center', justifyContent: 'center', marginVertical: 4 },
  legend: { marginTop: 10, gap: 3 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  legendDot: { width: 7, height: 7, borderRadius: 2 },
  legendText: { fontSize: 11, flex: 1 },
  numbersCard: { padding: 0 },
  numberRow: { flexDirection: 'row', alignItems: 'baseline', paddingVertical: 18, paddingHorizontal: 20, gap: 14 },
  numberKey: { flex: 1, fontSize: 14 },
  numberValue: { fontSize: 22, fontWeight: '500', letterSpacing: -0.3 },
  numberSub: { width: 80, textAlign: 'right', fontSize: 10 },
});
